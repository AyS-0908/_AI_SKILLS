...
1. **`skill-prompt-fable-5` has never been proven end-to-end.** Both runs were generator-side only; no prompt it wrote has been executed by Fable 5. A prompt to audit `skill-code-audit` is ready to paste at `high` effort. Until one runs, the skill's central bet — that 2-4 picked directives beat a hand-written prompt — is reasoned, not observed.
...
- `skill-prompt-fable-5` is unproven end-to-end (Next Action 1).
