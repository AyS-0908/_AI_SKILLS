...
- 2026-07-15: Created `skill-prompt-fable-5/` (LOCAL ONLY — not yet pushed). A prompt *generator*, not a Fable 5 manual: a cheaper model reads it, takes a task, and emits a paste-ready prompt + an effort recommendation for a fresh `claude-fable-5` / `claude-mythos-5` session. Source INPUT verified against the bundled `claude-api` skill: its one `<unverified>` item was REFUTED — minimum cacheable prefix is per-model, and Fable 5 is **2048** tokens, not 512 (Opus 4.8/4.7/4.6/4.5 + Haiku 4.5 = 4096; Sonnet 4.5 and older = 1024). `claude-mythos-5` confirmed real (Project Glasswing only, identical API surface). Refusals confirmed as HTTP **200** + `stop_reason: "refusal"` + `stop_details.category` (`cyber` / `bio` / `reasoning_extraction` / `frontier_llm` / `null`) — not an exception. Deliberately CUT from the source INPUT, because a pasted prompt cannot set them: client timeouts, server-side refusal fallbacks, `send_to_user` tool definition, skill-audit advice, and the whole API-parameter layer (`temperature`/`top_p`/`budget_tokens` all 400 on Fable 5 — irrelevant when writing text). Only `effort` survives, since the user picks it in the UI. Core design constraint is the model's own quirk: Fable 5 degrades under over-instruction, so the skill's central discipline is picking ≤4 directives from a trigger table rather than pasting all 13. **v2, same day (`d9eb4cc` shipped v1, defective):** the first real run — generating a prompt to audit `skill-code-audit` — exposed five miscalibrated trigger rows. The table had been written to *describe* the directives instead of to *select* them. `Outcome-first` was an always-on rule (per source) demoted to a conditional; `Context` was bundled with `Autonomous` when its real trigger is a token countdown; `Assess-only`'s text opened with a conditional about the user's conversational mode while its trigger fired on task type, so it sat inert on an audit request; `No-overbuild`'s trigger was effort-based but its text is pure software advice, wrong for review tasks; `Evidence` was scoped to "progress or status" when its value is any fact-asserting output. Fixed: every IF is now a yes/no question about the task, every directive text is an unconditional instruction, and the effort brake forks on code-vs-prose. Re-running the same task then produced a *better* prompt — `Assess-only` fired correctly and replaced a constraint the v1 run had forced to be hand-written, and `No-overbuild` was correctly excluded. **Rejected twice, on the record:** embedding the table in the generated prompt and letting Fable 5 self-select. It pastes every directive into context (over-instruction is measured by what is in the prompt, not by what the model decides is relevant), makes Fable 5's first act the option-survey `Act-once` exists to suppress, and — decisively — makes the prompt unauditable, since the user can only edit directives they can see, not ones chosen at runtime.
...
1. **`skill-prompt-fable-5` has never been proven end-to-end.** Both its runs were generator-side only; no prompt it wrote has yet been executed by Fable 5. A prompt to audit `skill-code-audit` is ready to paste at `high` effort. Until someone runs one and judges the result, the skill's central bet — that two-to-four picked directives beat a hand-written prompt — is reasoned from the source docs, not observed.
