...
description: >
  ...
  FRESH AI Auditor reviews it by invoking the code-audit skill (phase-gate mode)
  ...
  fix cycle (use code-audit or /code-review directly), ordinary single edits,
  non-phase work, or building the auditor itself.
---

# Code-Audit Loop

**Uses:** code-audit (hard dep), doc-hygiene, github-sync

Conductor skill. It does not audit and it does not invent a coder. It runs the
back-and-forth between a Coder and the existing **code-audit** skill until a phase
is green, with a hard round cap and a human final sign-off.

## Gotchas

- Skill type: RIGID conductor. Run ONE turn per invocation, update state, stop. Do not fast-forward the whole loop silently.
- The Auditor is `code-audit`, CALLED not copied. Never re-implement audit logic here. If `code-audit` is unavailable, stop and tell the user — do not fake an audit.
...
- Greenlight writes nothing to disk (code-audit contract). Detect the verdict by COUNTING `## Audit fixes to implement` sections in the plan before vs after the audit: one more section = Fix Plan; same count = Greenlight. Do not rely on the date heading — several rounds can share today's date. Record the result in state.
...
| **Auditor** | FRESH helper running `code-audit` | Review the phase, return Greenlight or a Fix Plan in the plan. |
...
- **Plan file**: the `*plan*.md` the Coder follows and `code-audit` writes fixes into. Ask once if unclear.
...
**Before round 1:** confirm the `code-audit` skill is available in this session and the exact plan file path is known. If `code-audit` is missing, STOP and tell the user — do not write any code (the loop is worthless without its auditor).
...
2. Spawn a FRESH helper at **best tier, effort 3/5** (see Models & effort). Give it the AUDITOR HANDOFF PROMPT, then: *"Use the code-audit skill in phase-gate mode. Scope = the changed files for this phase plus the Phase X slice of `<absolute plan path>`. Write any Fix Plan into that exact file. Return the verdict."* The main session runs the Coder at **best-1 @ 4/5** (jump to **best @ 4/5** for sensitive phases — security, secrets, login, money, data migration).
...
**Turn: DONE**
- Report to the user with proof (`usage_coding.md` close-out): phase is green, the checks that passed (evidence), the docs updated, and it awaits their final OK. Offer to commit the phase via `skill-github-sync`. The human gives the final sign-off — the loop does the grunt work, not the decision.
...
- **Role** — You are an AI Auditor. Use the code-audit skill in phase-gate mode.
...
- **Human final sign-off**: the loop reaches Greenlight; the user confirms ship. Never mark the phase shipped on the Auditor's word alone.

## State and Resume

State lives in `code-audit-loop/state.json` in the project. The Fix Plans live
in the implementation plan (code-audit owns those). Round/turn/verdict live in state.
...
- `code-audit` is not available in this session,
